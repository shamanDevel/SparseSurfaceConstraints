...
public:
	...
	/// `f(x, grad)` returns the objective function value at `x` and overwrites `grad` with the gradient.
	///
	typedef std::function<Scalar(const Vector& x, Vector& grad)> ObjectiveFunction_t;

	///
	/// Callback function
	///
	/// x: current parameters `x`
	/// g: current gradient `\nabla x`
	/// v: current value of f(x)
	/// k: number of iterations
	///
	/// Return true iff the execution should continue
	///
	typedef std::function<bool(const Vector& x, const Vector& g, const Scalar& v, int k)> CallbackFunction_t;

    ///
    /// Function that validates a given point.
    /// It can be used to check if the search is within certain external bounds.
    /// Note, this is used in the Line Search algorithm to limit the step size,
    /// the gradient must still point away from these bounds.
    ///
    /// x: current position `x`
    ///
    /// Return true iff this position is within some external bounds
    ///
    typedef std::function<bool(const Vector& x)> ValidationFunction_t;
    ...
    ///
    int minimize(const ObjectiveFunction_t& f, Vector& x, Scalar& fx,
		const CallbackFunction_t& callback = {},
        const ValidationFunction_t& validation = {})
    {
		...
		if (!callback(x, m_grad, fx, 1)) return 1;
        ...
        for( ; ; )
        {
            ...
            LineSearch<Scalar>::Backtracking(f, fx, x, m_grad, step, m_drt, m_xp, m_param, validation);

			if (!callback(x, m_grad, fx, k)) return k;
