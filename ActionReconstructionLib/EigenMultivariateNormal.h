...
/*
  ...
  Source: https://github.com/beniz/eigenmvn/blob/master/eigenmvn.h
*/
namespace Eigen {
  namespace internal {
  ...
  */
    ...
    class EigenMultivariateNormal
  {
    ...
    bool _use_cholesky;
    SelfAdjointEigenSolver<Matrix<Scalar,Size,Size> > _eigenSolver; // drawback: this creates a useless eigenSolver when using Cholesky decomposition, but it yields access to eigenvalues and vectors
  ...
  EigenMultivariateNormal(const Matrix<Scalar,Size,1>& mean,const Matrix<Scalar,Size,Size>& covar,
	...
	{
	  _eigenSolver = SelfAdjointEigenSolver<Matrix<Scalar,Size,Size> >(_covar);
    ...
    Matrix<Scalar,Size,Dynamic> samples(int nn)
      {
	return (_transform * Matrix<Scalar,Size,Dynamic>::NullaryExpr(_covar.rows(),nn,randN)).colwise() + _mean;
      }
